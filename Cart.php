...
<DOCTPYE html>
        ...
        <div class="flex items-center">
          ...
          <div class="hidden md:block">
            <div class="ml-10 flex items-baseline space-x-4">
              ...
              <a href="Cart.php" class="bg-indigo-800 text-white rounded-md px-3 py-2 text-sm font-medium">Cart</a>
  ...
  <main>
    ...
    if (isset($_SESSION['tours'])){
        ...
        echo "<h1>No current Tours</h1>";
    }
    ?>
    <ul>
            <?php foreach ($_SESSION['tours'] as $index => $tour) : 
                $tourtotal = $tour[4] * $tour[3];
                (int)$totaltours += (int)$tourtotal;

                $_SESSION['Checkout'] = [$totaltours,$userName,$tour[3]];
                ?>
    
                <li>
                    <strong>Location:</strong> <?php echo $tour[0]; ?><br>
                    <strong>Price:</strong> $<?php echo number_format($tour[4], 2); ?><br>
                    <strong>Number of travelers:</strong> <?php echo $tour[3]; ?><br>
                    <form action="RemovefromCart.php" method="POST">
                        <input type="hidden" name="location" value="<?php echo $tour[0]; ?>">
                        <button type="submit" class="bg-indigo-800 text-white rounded-md px-3 py-2 text-sm font-medium" >Remove from Cart</button>
                    </form>
                </li>
            <?php endforeach; ?>
            <strong>Total:$</strong> <?php echo $totaltours; ?><br>
            <form action="Checkout.php" method="POST">
                <input type="hidden" name="total" value="<?php echo $totaltours; ?>">
                <Button class="bg-indigo-800 text-white rounded-md px-3 py-2 text-sm font-medium">Checkout</Button>
            </form>
    </div>
  </main>
</div>
