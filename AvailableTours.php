<?php  
    ...
    $sql = "SELECT * FROM Tours WHERE Capacity > registerednum";
...
<DOCTPYE html>
        ...
        <div class="flex items-center">
          ...
          <div class="hidden md:block">
            <div class="ml-10 flex items-baseline space-x-4">
              ...
              <a href="AvailableTours.php" class="bg-indigo-800 text-white rounded-md px-3 py-2 text-sm font-medium">Available tours</a>
  ...
  <header class="bg-white shadow">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <h1 class="text-3xl font-bold tracking-tight text-gray-900">Available Tours</h1>
    </div>
  </header>
  <main>
      <?php
            while ($row = $result->fetch_assoc()) {
                ...
                $available = $capacity - $registerednum;
                echo "
                      <h1>Trip to $Location,   Date:$Date,   Positions available:$available,   Price:$$Price</h1>
                      <form action='AddtoCart.php' method='POST'>
                        <input type='hidden' name='Location' value='$Location'>
                        <input type='hidden' name='available' value='$available'>
                        <label>Number of Travelers</label>
                        <input type='number' name='numtravelers' style='outline: 2px solid #9400D3;'>
